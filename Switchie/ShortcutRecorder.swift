...
private final class RecordingCoordinator: ObservableObject {
    ...
    func begin(for picker: ShortcutPickerHandle) {
        if let current = activeID, current != picker.recorderID {
            NotificationCenter.default.post(name: .switchieCancelOtherPicker, object: current)
        }
        activeID = picker.recorderID
        NotificationCenter.default.post(name: .switchieBeginRecording, object: nil)
    }

    func end(for picker: ShortcutPickerHandle) {
        guard activeID == picker.recorderID else { return }
        activeID = nil
        NotificationCenter.default.post(name: .switchieEndRecording, object: nil)
    }
...
struct ShortcutPicker: View, ShortcutPickerHandle {
    ...
    var body: some View {
        ...
        .background(RecorderEventCatcher(
            ...
            onCommit: { new in
                ...
                NotificationCenter.default.post(
                    name: .switchieShortcutCommitted,
                    object: nil,
        ...
        .onReceive(NotificationCenter.default.publisher(for: .switchieCancelOtherPicker)) { note in
            if let id = note.object as? UUID, id == recorderID {
                cancelRecordingAndRevert()
            }
        }
        .onReceive(NotificationCenter.default.publisher(for: .switchieShortcutCommitted)) { note in
...
extension Notification.Name {
    static let switchieCancelOtherPicker  = Notification.Name("SwitchieCancelOtherPicker")
    static let switchieShortcutCommitted  = Notification.Name("SwitchieShortcutCommitted")
}
