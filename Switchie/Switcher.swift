...
/// How the app-toggle shortcut behaves on tap.
enum AppSwitchMode: String, CaseIterable, Identifiable {
    case cycle, toggle
...
final class Switcher: ObservableObject, OverlayDelegate {
    ...
    @Published private(set) var onlyCycleMarkedApps: Bool
    @Published private(set) var appSwitchMode: AppSwitchMode
    ...
    init() {
        ...
        onlyCycleMarkedApps = UserDefaults.standard.object(forKey: Preferences.Key.onlyMarkedApps) as? Bool ?? false

        let modeRaw = UserDefaults.standard.string(forKey: Preferences.Key.appSwitchMode) ?? AppSwitchMode.cycle.rawValue
        appSwitchMode = AppSwitchMode(rawValue: modeRaw) ?? .cycle

        windowCycleShortcut     = Preferences.loadShortcut(forKey: Preferences.Key.windowCycle,     default: Preferences.Default.windowCycle)
        overlaySelectShortcut   = Preferences.loadShortcut(forKey: Preferences.Key.overlaySelect,   default: Preferences.Default.overlaySelect)
        overlayQuitShortcut     = Preferences.loadShortcut(forKey: Preferences.Key.overlayQuit,     default: Preferences.Default.overlayQuit)
        overlayMarkShortcut     = Preferences.loadShortcut(forKey: Preferences.Key.overlayMark,     default: Preferences.Default.overlayMark)
        separateToggleShortcut  = Preferences.loadShortcut(forKey: Preferences.Key.separateToggle,  default: Preferences.Default.separateToggle)
        separateOverlayShortcut = Preferences.loadShortcut(forKey: Preferences.Key.separateOverlay, default: Preferences.Default.separateOverlay)
    ...
    func start() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(beginShortcutRecording), name: .switchieBeginRecording, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(endShortcutRecording), name: .switchieEndRecording, object: nil)
    }
    ...
    func setOnlyCycleMarkedApps(_ enabled: Bool) {
        guard enabled != onlyCycleMarkedApps else { return }
        onlyCycleMarkedApps = enabled
        UserDefaults.standard.set(enabled, forKey: Preferences.Key.onlyMarkedApps)
        cycleSnapshot = nil
        cycleIndex = 0
    }

    func setAppSwitchMode(_ mode: AppSwitchMode) {
        guard mode != appSwitchMode else { return }
        appSwitchMode = mode
        UserDefaults.standard.set(mode.rawValue, forKey: Preferences.Key.appSwitchMode)
        cycleSnapshot = nil
        cycleIndex = 0
    }
    ...
    private func cycleToNextApp() {
        // "Only cycle through marked apps" forces cycle behavior — but only
        // when there's actually a marked set to cycle through. With no marks,
        // fall back to the user's chosen app-switch mode.
        let hasMarkedApps = !Preferences.markedBundleIDs.isEmpty
        let useMarkedOverride = onlyCycleMarkedApps && hasMarkedApps

        if useMarkedOverride || appSwitchMode == .cycle {
            cycleThroughList()
        } else {
            toggleBetweenRecentApps()
        }
    }
    ...
    private func toggleBetweenRecentApps() {
        ...
        // even before the workspace activation notification fires.
    ...
    private func effectiveCycleList() -> [NSRunningApplication] {
        pruneMRU()
        guard onlyCycleMarkedApps else { return mru }

        let marked = Preferences.markedBundleIDs
        guard !marked.isEmpty else { return mru }
...
extension Notification.Name {
    static let switchieBeginRecording = Notification.Name("SwitchieBeginShortcutRecording")
    static let switchieEndRecording   = Notification.Name("SwitchieEndShortcutRecording")
}
